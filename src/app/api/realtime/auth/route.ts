...
import { getPusherServer, parsePresenceChannel, parsePrivateChannel } from '@/lib/pusher-server';
...
export async function POST(request: Request) {
  try {
    ...
    const tripId = parsePresenceChannel(channelName) ?? parsePrivateChannel(channelName);
    if (!tripId) {
    ...
    if (parsePresenceChannel(channelName)) {
